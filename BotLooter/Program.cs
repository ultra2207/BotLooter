...
{
    Log.Logger.Fatal((Exception)eventArgs.ExceptionObject, "Exception! You can contact the developer directly for clarification or open an issue on GitHub.");
...
async Task<IRestClientProvider?> GetClientProvider()
{
    ...
    else
    {
        ...
        {
            FlowUtils.AbortWithError($"No proxies found in '{config.ProxiesFilePath}'");
            return null;
        }
    ...
    if (config.LootThreadCount > clientProvider.AvailableClientCount)
    {
        switch (clientProvider)
        {
            case ProxyRestClientProvider:
                Log.Logger.Warning("Threads {ThreadCount} exceed available proxies {ClientCount}. The number of threads will be reduced to match the number of proxies.", config.LootThreadCount, clientProvider.AvailableClientCount);
                break;
