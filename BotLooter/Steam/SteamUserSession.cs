    ...
    private async Task<(bool Success, string Message)> TryLogin()
    {
        try
        {
            ...
            if (loginSession.RefreshToken is null)
            {
                ...
                {
                    return (false, $"Failed to login: {loginResult.Message} {(loginResult.XEResult is null ? "" : $"({loginResult.XEResult})")}");
                }
            }
            ...
            {
                return (false, $"Failed to get web cookies: {getCookiesResult.Message}");
            }
        ...
        catch
        {
            ...
            return (false,
                $"Failed to login, possibly an issue with Steam or {(isUsingProxy ? "proxy" : "internet")}");
        }
    }
